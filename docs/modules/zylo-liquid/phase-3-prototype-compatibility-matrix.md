...
| 7 | Caisse, encaissements, transactions de shift, clôture, quarts (3627-3701, 4971-5108) | Ventes, moyens de paiement, quarts pompistes | Aucun modèle caisse/POS/pompiste au Niveau 1 (confirmé absent en Phase 1, §20.7 — tables `users`/`user_station_access` encore "à valider", pas de table transaction) | NE PAS IMPLÉMENTER (hors périmètre) | Absent |
| 8 | Contribution non pétrolière / boutique (3748) | CA boutique, lavage, vidange | `Station.hasShop/shopName/shopSurfaceM2` = descriptif seul, aucune donnée de revenu | PRÉSENTER MAIS DÉSACTIVER | Absent |
...
| 13b | ↳ Documents réglementaires, sécurité incendie/ATEX (3972-3975) | Documents, zones | Aucun modèle | NE PAS IMPLÉMENTER (hors périmètre) | Absent |
...
| 21 | Réconciliation par cuve, décomposition du calcul (4871-4924) | Stock théorique vs réel (livraisons − ventes − stock) | Ventes individuelles absentes (point 5/7) ; livraisons + mesures disponibles mais réconciliation complète impossible sans les sorties | PRÉSENTER MAIS DÉSACTIVER | Absent |
| 22 | Ventes par produit, dernières transactions (4961-4971) | Volumes vendus, tickets | Aucune donnée de vente individuelle | NE PAS IMPLÉMENTER (hors périmètre) | Absent |
...
| 26 | HSE, incendie, maintenance, inspections, contrats (5380-5526) | Équipements sécurité, interventions, contrats | Aucun modèle | NE PAS IMPLÉMENTER (hors périmètre) | Absent |
| 27 | Comptes clients à crédit, grand livre, relevé (5643-5836) | Solde, mouvements, échéances | Couche 7 "Crédit client" du schéma cible — confirmée non construite (`phase-1-database.md` : couches 3-7 en attente de décision Core/module) | IMPLEMENTER APRÈS EXTENSION (nouvelle couche complète, nécessite décision architecturale explicite avant toute table) | Absent |
...
- **IMPLEMENTER APRÈS EXTENSION** : jaugeage manuel (#15, nouvel endpoint de correction), historique de sync détaillé (#28), scope RBAC par station (#30), matrice rôles×permissions (#37).
...
- **PRÉSENTER MAIS DÉSACTIVER** : mode dégradé (#2), contribution boutique (#8), incidents sécurité (#10), équipements/personnel station (#13a), catalogue de traitement (#16), fiabilité fournisseur (#17), réconciliation stock complète (#21).
- **HORS PÉRIMÈTRE (crédit client)** : comptes clients à crédit (#27) — couche 7 du schéma cible, nécessite une décision architecturale explicite avant toute table (pas tranchée ici, cf. `phase-1-database.md` §20.7).
- **NE PAS IMPLÉMENTER (hors périmètre confirmé)** : tout ce qui dépend de ventes/transactions individuelles, de distributeurs, de personnel/quarts, de commandes fournisseurs, de HSE/maintenance/incendie, de configuration/feature-flags — Niveau 1 est une télémétrie de cuves, pas un POS ni un ERP RH/maintenance.
